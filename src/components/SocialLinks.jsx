  ...
  return (
    <div className="fixed bottom-20 right-6 z-40 hidden lg:block">
      <div className="flex flex-col gap-3">
        {links.map(({ id, icon, href, label, style, download, isScroll }) => {
          if (isScroll) {
            return (
              <Link
                key={id}
                ...
                smooth
                duration={500}
                className={`group relative flex items-center justify-center w-12 h-12 rounded-full ${style} shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-125 opacity-70 hover:opacity-100 cursor-pointer`}
                aria-label={label}
              >
                {/* Icon */}
